    ...
    def write(self):
        ...
        with open(self.filename, "wb") as stat_fh:
            for data in self._data.values():
                stat_fh.write(repr(data).encode("ascii") + "\n")
                ...
                if matches:
                    ...
                    for cont_pattern in cont_patterns:
                        if isinstance(cont_pattern, int):  # add that many lines
                            for _unused_x in xrange(cont_pattern):
...
def _decode_to_unicode(match):
    ...
    return match.decode('utf-8', 'replace') if isinstance(match, str) else match
...
def _filter_maxcontextlines(lines_list, before, after):
    ...
    indices = iter(xrange(-before, n_lines))
    context_end = -1
    for idx in indices:
