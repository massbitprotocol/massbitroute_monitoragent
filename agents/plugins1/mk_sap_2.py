...
def process_alerts(cfg_entry, logs, ms_name, mon_name, node, alerts):
    ...
    for alert in alerts:
        ...
        logs[hostname][logfile].append(
            '%s %s %s' %
            (STATE_LOGWATCH_MAP[alert_state['VALUE']], dt.strftime("%Y-%m-%d %H:%M:%S"), alert_msg))
        ...
        for node in tree:
            ...
            if node['MTCLASS'] == MTE_PERFORMANCE:
                ...
                if len(alerts) > 0:
                    ...
                    last_msg = '%s: %s - %s' % (dt, STATE_VALUE_MAP[alert_state['VALUE']][1],
                                                alert_msg)
            ...
            if node['MTCLASS'] not in SKIP_MTCLASSES:
                ...
                sap_data[sid].append("%s\t%d\t%3d\t%s\t%s\t%s\t%s" %
                                     (context, state['VALUE'], state['SEVERITY'], path, perfvalue,
                                      uom, status_details))
